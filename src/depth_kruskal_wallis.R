...
st_depth_boxpl$Scaffold_full_id <- as.factor(st_depth_boxpl$Scaffold_full_id)
test_res <- kruskal.test(depth ~ Scaffold_full_id, data=st_depth_boxpl)
chars <- capture.output(print(test_res))
writeLines(chars, con=file(snakemake@output[["kw_res"]]))

pwt <- pairwise.wilcox.test(st_depth_boxpl$depth, st_depth_boxpl$Scaffold_full_id,
                 p.adjust.method = "BH")
chars <- capture.output(print(pwt))
writeLines(chars, con=file(snakemake@output[["pw_res"]]))